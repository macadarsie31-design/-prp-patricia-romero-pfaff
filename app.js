...
function buildSuggestedPlan() {
  const zones = state.fullFace
    ? ["undereye", "midface", "cheeks", "perioral", "lips", "chin", "wrinkles"]
    : [...state.areas];
  ...
  for (const zone of zones) {
    const item = TREATMENT_PLAN[zone];
    if (!item) continue;

    if (Array.isArray(item.filler)) filler.push(...item.filler);
    if (Array.isArray(item.botox)) botox.push(...item.botox);
    if (Array.isArray(item.threads)) threads.push(...item.threads);
  }

  state.plan = { filler, botox, threads };
}
...
function renderTreatmentPanel() {
  ...
  if (!panel) {
    ...
    const compare = $("#compare");
    if (compare) {
      compare.insertAdjacentElement("afterend", panel);
    } else {
      $("#simWorkspace")?.appendChild(panel);
    }
  }

  const fillerHtml = state.plan.filler.length
    ? state.plan.filler.map(i =>
        `<li><strong style="display:inline;font-size:inherit;letter-spacing:0">${escapeHtml(i.area)}:</strong> ${escapeHtml(i.ml)}</li>`
      ).join("")
    : "<li>No indicado en esta simulación</li>";

  const botoxHtml = state.plan.botox.length
    ? state.plan.botox.map(i => `<li>${escapeHtml(i.area)}</li>`).join("")
    : "<li>No indicado en esta simulación</li>";

  const threadsHtml = state.plan.threads.length
    ? state.plan.threads.map(i => `<li>${escapeHtml(i.area)}</li>`).join("")
    : "<li>No indicado en esta simulación</li>";

  panel.innerHTML = `
    <h3>Plan sugerido de armonización</h3>

    <div class="prp-treatment-group">
      <strong>FILLER · RANGO ORIENTATIVO</strong>
      <ul>${fillerHtml}</ul>
    </div>

    <div class="prp-treatment-group">
      <strong>BOTULINUM TOXIN</strong>
      <ul>${botoxHtml}</ul>
    </div>
    ...
    <div class="prp-treatment-disclaimer">
      Estimación visual orientativa basada únicamente en la fotografía frontal.
      Los procedimientos, productos y cantidades definitivas deben confirmarse
      mediante evaluación clínica presencial por la profesional tratante.
    </div>
  `;
}
