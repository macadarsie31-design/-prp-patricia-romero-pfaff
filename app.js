(() => {
"use strict";
console.info("PRP simulator build V17-final-refined");
...
const PROFILE_CONFIG = {
  natural: {
    ...
    shortStatus: "Natural",
    recommendation: "Cambios sutiles y conservadores: mirada más descansada, suavizado leve de líneas, soporte armónico y labios discretamente más hidratados, sin transformar rasgos.",
    note: "Simulación orientativa basada en una fotografía frontal y métricas geométricas. En modo Natural se priorizan ajustes suaves, conservando al máximo tus rasgos y mostrando una mejoría delicada.",
    planScale: 0.85,
    blendCap: 0.78,
    liveReady: "Simulación Natural lista · ajustá la intensidad"
  },
  best: {
    label: "Mejor versión",
    title: "Mejor versión",
    shortStatus: "Mejor versión",
    recommendation: "Cambios más notorios pero elegantes: menos arrugas visibles, mirada más abierta y descansada, labios con más hidratación y volumen sutil, y una apariencia más armónica y juvenil preservando tu identidad.",
    note: "Simulación orientativa basada en una fotografía frontal y métricas geométricas. En modo Mejor versión se priorizan cambios visibles pero armónicos, buscando un resultado más joven, descansado y refinado.",
    planScale: 1.08,
    blendCap: 0.92,
...
function buildControls() {
  ...
  if (profileBlock && !$("#prpAiLive")) {
    ...
    note.textContent = PROFILE_CONFIG[state.profile].note;
...
function updateProfileTexts() {
  ...
  if (live && !state.aiReady) live.textContent = `Modo ${cfg.label} seleccionado · actualizá la simulación`;
}
...
function markSimulationDirty() {
  ...
  const live = $("#prpAiLive");
  if (live) {
    live.textContent = `Modo ${PROFILE_CONFIG[state.profile].label} seleccionado · actualizá la simulación`;
...
function buildSuggestedPlan() {
  ...
  addJuvederm(
    ...
    state.profile === "best" ? 0.5 : 0.3, state.profile === "best" ? 1.1 : 0.8,
  ...
  addJuvederm(
    ...
    state.profile === "best" ? 0.5 : 0.2, state.profile === "best" ? 0.9 : 0.5,
  ...
  if ((m.jawToFaceWidth || 0) < 0.74 || (m.transverseAsymmetry || 0) > 0.08 || state.profile === "best") {
    addJuvederm(
      "Pre-jowl / continuidad mandibular",
      0.5, state.profile === "best" ? 1.4 : 1.0,
  ...
  botox.push({ area: "Cola de ceja / brow tail", recommendation: state.profile === "best" ? "Valorar elevación sutil de cola de ceja para una mirada más abierta, almendrada y armónica." : "Valorar elevación muy sutil de cola de ceja solo si mantiene naturalidad." });
  ...
  threads.push({ area: "Hilos tensores / efecto lifting", recommendation: state.profile === "best" ? "Valorar elevación sutil de cola de ceja, tercio medio y contorno mandibular si la profesional considera que mejora la armonía sin exageración." : "No indicados por defecto. Valorar solo si aportan un beneficio claro, suave y natural." });
...
function renderTreatmentPanel() {
  ...
  panel.innerHTML = `
    ...
    <div style="margin:0 0 16px;padding:12px 14px;border-radius:14px;background:#f7f4ef;color:#5e5751">${escapeHtml(PROFILE_CONFIG[state.profile].recommendation)}</div>
...
async function generateAiBestVersion() {
  ...
  try {
    ...
    if (live) live.textContent = PROFILE_CONFIG[state.profile].liveReady;
...
function render() {
  ...
  if (state.aiReady && state.aiCanvas) {
    const blendCap = PROFILE_CONFIG[state.profile]?.blendCap ?? 1;
