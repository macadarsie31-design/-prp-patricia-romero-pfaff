...
async function init(){
  ...
  setStatus("Cargando motor facial…");
  setUploadEnabled(false);

  state.enginePromise = ensureEngineReady();
  state.enginePromise
    .then(() => {
      setUploadEnabled(true);
      setStatus("Motor facial listo · subí una selfie.");
    })
    .catch(err => {
      console.error("Engine initialization failed", err);
      setUploadEnabled(true);
      setStatus("No se pudo iniciar el motor. Tocá Subir foto para reintentar.");
    });
}
...
function setUploadEnabled(enabled){
  [$("#fileInput"),$("#fileInput2")].filter(Boolean).forEach(input => {
    ...
    if(label){
      label.style.opacity = enabled ? "1" : ".55";
...
function loadScript(url, timeoutMs=15000){
  return new Promise((resolve,reject)=>{
    ...
    const timer=setTimeout(()=>{s.remove();reject(new Error("Timeout cargando "+url));},timeoutMs);
    s.onload=()=>{clearTimeout(timer);s.dataset.loaded="1";resolve();};
    s.onerror=()=>{clearTimeout(timer);s.remove();reject(new Error("No se pudo cargar "+url));};
    document.head.appendChild(s);
  });
}

async function loadFirstWorking(urls, testFn, label){
  let lastErr=null;
  for(const url of urls){
    try{
      await loadScript(url);
      if(testFn()) return;
      throw new Error(label+" no quedó disponible");
    }catch(err){
      lastErr=err;
      console.warn(label+" fallback failed:",url,err);
    }
  }
  throw lastErr || new Error("No se pudo cargar "+label);
}

async function ensureEngineReady(){
  if(state.detectorReady && state.Delaunator) return true;

  setStatus("Cargando detector facial…");

  await loadFirstWorking([
    "https://cdn.jsdelivr.net/npm/@mediapipe/face_mesh@0.4.1633559619/face_mesh.js",
    "https://unpkg.com/@mediapipe/face_mesh@0.4.1633559619/face_mesh.js"
  ],()=>typeof window.FaceMesh==="function","MediaPipe FaceMesh");

  setStatus("Cargando triangulación facial…");

  await loadFirstWorking([
    "https://cdn.jsdelivr.net/npm/delaunator@5.1.0/delaunator.min.js",
    "https://unpkg.com/delaunator@5.1.0/delaunator.min.js"
  ],()=>typeof window.Delaunator!=="undefined","Delaunator");

  state.Delaunator = window.Delaunator;

  if(!state.faceMesh){
    const versionBase="https://cdn.jsdelivr.net/npm/@mediapipe/face_mesh@0.4.1633559619/";
    state.faceMesh = new window.FaceMesh({
      locateFile: file => versionBase + file
    });
    ...
    state.faceMesh.onResults(results=>{
      ...
      if(state.pendingFaceResolve){
        const resolve=state.pendingFaceResolve;
        state.pendingFaceResolve=null;
        state.pendingFaceReject=null;
        resolve(lm);
      }
    });
  }
...
async function detectFaceOnCanvas(canvas){
  await ensureEngineReady();
  return new Promise(async (resolve,reject)=>{
    const timeout=setTimeout(()=>{
      if(state.pendingFaceReject){
        state.pendingFaceResolve=null;
        state.pendingFaceReject=null;
        reject(new Error("El análisis facial tardó demasiado."));
      }
    },20000);

    state.pendingFaceResolve = lm => { clearTimeout(timeout); resolve(lm); };
    state.pendingFaceReject = err => { clearTimeout(timeout); reject(err); };

    try{
      await state.faceMesh.send({image:canvas});
    }catch(err){
...
function buildControls(){
  ...
  const live=document.createElement("div");live.className="v11-live";live.id="v11Live";live.textContent="Motor local · sin costo por movimiento";
...
function bindUpload(){
  [$("#fileInput"),$("#fileInput2")].filter(Boolean).forEach(input=>input.addEventListener("change",async e=>{
    const file=e.target.files?.[0]; if(!file)return;
    if(!file.type.startsWith("image/"))return alert("Elegí una foto.");

    try{
      setStatus("Preparando motor facial…");
      if(!state.enginePromise) state.enginePromise=ensureEngineReady();
      await state.enginePromise;
    }catch(err){
      console.error(err);
      setStatus("Reintentando carga del motor…");
      state.enginePromise=ensureEngineReady();
      try{ await state.enginePromise; }
      catch(err2){
        console.error(err2);
        alert("No pudimos cargar el escáner facial. Revisá la conexión y tocá Subir foto nuevamente.");
        return;
      }
    }
...
async function startImage(img){
  try{
    setStatus("Preparando análisis facial…");
    await ensureEngineReady();
  }catch(err){
    console.error(err);
    alert("No se pudo cargar el escáner facial. Probá nuevamente con conexión estable.");
    return;
  }
  ...
  let lm;
  try{
    lm = await detectFaceOnCanvas(source);
  }catch(e){
    console.error(e);
    alert("No se pudo analizar esta foto. Probá nuevamente.");
    return;
  }
  if(!lm||lm.length<468){
    alert("No pude detectar el rostro con suficiente precisión. Usá una foto frontal o 3/4, con la cara completa y buena luz.");
    return;
  }
...
window.addEventListener("online",()=>{
  if(!state.detectorReady){
    state.enginePromise=ensureEngineReady();
    state.enginePromise.then(()=>{setUploadEnabled(true);setStatus("Motor facial listo · subí una selfie.");}).catch(console.error);
  }
});
})();
