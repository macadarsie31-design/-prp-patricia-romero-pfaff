(() => {
  ...
  function requestLiveRender() {
    ...
    renderRAF = requestAnimationFrame(() => {
      ...
      renderProposal();
      if ($("#simStatus")) $("#simStatus").textContent = "Vista en vivo";
    });
  }
  ...
  async function startWithImage(img) {
    ...
    // Show an immediate live preview, then refine it once face landmarks load.
    state.generated = true;
    renderProposal();
    if ($("#simStatus")) $("#simStatus").textContent = "Previsualización activa";

    detectFace().then(() => {
      if (state.img) {
        renderProposal();
        if ($("#simStatus")) $("#simStatus").textContent = "Rostro analizado · vista en vivo";
      }
    }).catch(() => {
      if ($("#simStatus")) $("#simStatus").textContent = "Vista en vivo";
    });
  ...
  function renderProposal() {
      ...
      switch (state.zone) {
        case "lips": {
          ...
          tintEllipse(ctx, (lipL.x+lipR.x)/2, (lipTop.y+lipBottom.y)/2,
            lipWidth*.56, lipHeight*.30, `rgba(170,75,78,${0.09*s})`);
          break;
        }
        case "cheeks":
          ...
          brightenEllipse(ctx,leftCheek.x,leftCheek.y,faceWidth*.12,faceWidth*.09,.09*s);
          brightenEllipse(ctx,rightCheek.x,rightCheek.y,faceWidth*.12,faceWidth*.09,.09*s);
          break;
        case "chin":
        ...
        case "jaw":
          ...
          shadeEllipse(ctx, pt(172).x, pt(172).y, faceWidth*.10, faceWidth*.16, .03*s);
          shadeEllipse(ctx, pt(397).x, pt(397).y, faceWidth*.10, faceWidth*.16, .03*s);
          break;
        case "undereye":
          brightenEllipse(ctx,pt(145).x,pt(145).y,faceWidth*.085,faceWidth*.043,.16*s);
          brightenEllipse(ctx,pt(374).x,pt(374).y,faceWidth*.085,faceWidth*.043,.16*s);
          break;
        case "skin":
          softSkin(ctx, after, .09*s);
          brightenEllipse(ctx, after.width*.5, after.height*.42, faceWidth*.36, faceWidth*.46, .04*s);
          break;
      }
    } else {
    ...
    after.dataset.previewReady = "true";
  }

  function fallbackByZone(ctx, c, zone, s) {
    const w=c.width, h=c.height;
    if(zone==="skin") softSkin(ctx,c,.09*s);
    if(zone==="undereye"){
      brightenEllipse(ctx,w*.40,h*.42,w*.075,h*.025,.14*s);
      brightenEllipse(ctx,w*.60,h*.42,w*.075,h*.025,.14*s);
    }
    if(zone==="lips"){
      localPatchScale(ctx,c,w*.5,h*.64,w*.25,h*.10,1+.055*s,1+.14*s);
    }
    if(zone==="cheeks"){
      brightenEllipse(ctx,w*.36,h*.54,w*.10,h*.07,.09*s);
      brightenEllipse(ctx,w*.64,h*.54,w*.10,h*.07,.09*s);
    }
    if(zone==="chin") localPatchScale(ctx,c,w*.5,h*.75,w*.22,h*.14,1+.01*s,1+.09*s);
    if(zone==="jaw"){
      shadeEllipse(ctx,w*.34,h*.70,w*.10,h*.16,.03*s);
      shadeEllipse(ctx,w*.66,h*.70,w*.10,h*.16,.03*s);
    }
  }
  ...
  function subtleJawContour(ctx,c,left,right,chin,fw,s){
    ctx.save();
    ctx.strokeStyle=`rgba(105,78,62,${.11*s})`;
