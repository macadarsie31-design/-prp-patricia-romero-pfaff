...
const TREATMENT_PLAN = {
  undereye: {
    filler: [{ area: "Ojeras / surco lagrimal", ml: "0.2–0.5 mL por lado" }],
    botox: [],
    threads: []
  },
  temples: {
    filler: [{ area: "Sienes", ml: "0.3–0.7 mL por lado" }],
    botox: [],
    threads: []
  },
  midface: {
    filler: [{ area: "Soporte tercio medio", ml: "0.5–1.0 mL por lado" }],
    botox: [],
    threads: [{ area: "Soporte sutil del tercio medio, si está indicado" }]
  },
  cheeks: {
    filler: [{ area: "Pómulos", ml: "0.4–0.8 mL por lado" }],
    botox: [],
    threads: []
  },
  nose: {
    filler: [{ area: "Nariz — refinamiento no quirúrgico", ml: "0.2–0.4 mL total" }],
    botox: [],
    threads: []
  },
  perioral: {
    filler: [{ area: "Nasolabial / perioral", ml: "0.3–0.7 mL por lado" }],
    botox: [{ area: "DAO, si está indicado" }],
    threads: []
  },
  lips: {
    filler: [{ area: "Labios", ml: "0.4–0.8 mL total" }],
    botox: [],
    threads: []
  },
  chin: {
    filler: [{ area: "Mentón", ml: "0.5–1.5 mL total" }],
    botox: [{ area: "Mentalis, si está indicado" }],
    threads: []
  },
  prejowl: {
    filler: [{ area: "Pre-jowl", ml: "0.3–0.7 mL por lado" }],
    botox: [],
    threads: [{ area: "Soporte leve de línea mandibular, si está indicado" }]
  },
  jaw: {
    filler: [{ area: "Mandíbula", ml: "0.5–1.0 mL por lado" }],
    botox: [{ area: "Masetero, si está indicado" }],
    threads: [{ area: "Definición mandibular sutil, si está indicado" }]
  },
  wrinkles: {
    filler: [],
    botox: [
...
function renderScanPanel() {
  ...
  const m = a.metrics;
  const kimiState = state.kimiStatus || "idle";
  const kimiNote =
    kimiState === "ready"
      ? `<div class="prp-scan-kimi">Kimi analizó la selfie y generó un plan estructurado.</div>`
      : kimiState === "loading"
      ? `<div class="prp-scan-kimi">Kimi está interpretando la foto y armando el plan clínico…</div>`
      : kimiState === "fallback"
      ? `<div class="prp-scan-warning">Kimi no respondió. Se usará el plan local de respaldo.</div>`
      : "";

  panel.innerHTML = `
    <strong>Escaneo estructural · ${escapeHtml(a.landmarkCount)} landmarks</strong>
    MediaPipe analiza proporciones y simetría antes de generar la simulación.
...
async function startImage(img) {
  ...
  state.kimiStatus = "loading";
  renderScanPanel();
  setStatus("Kimi está analizando la selfie…");

  try {
    state.kimiPlan = await analyzeWithKimi();
    state.kimiStatus = state.kimiPlan ? "ready" : "fallback";
  } catch (kimiError) {
    console.error("Kimi analysis failed:", kimiError);
    state.kimiPlan = null;
    state.kimiStatus = "fallback";
  }
...
async function analyzeWithKimi() {
  if (!state.sourceImageDataUrl || !state.faceAnalysis) return null;
  ...
  if (!response.ok || !data?.ok || !data?.plan) {
    const err = new Error(data?.error || `Kimi error ${response.status}`);
    err.payload = data;
    throw err;
  }
...
function buildSuggestedPlan() {
  const kimi = state.kimiPlan;

  if (kimi && kimi.procedures) {
    const filler = Array.isArray(kimi.procedures.filler) ? kimi.procedures.filler.map(item => ({
      area: item.area || item.name || "Área",
      ml: item.volume_note || item.note || item.rationale || "Evaluación clínica presencial"
    })) : [];

    const botox = Array.isArray(kimi.procedures.botox) ? kimi.procedures.botox.map(item => ({
      area: item.area || item.name || item.rationale || "Área"
    })) : [];

    const threads = Array.isArray(kimi.procedures.threads) ? kimi.procedures.threads.map(item => ({
      area: item.area || item.name || item.rationale || "Área"
    })) : [];

    const mdCodes = [
      ...(Array.isArray(kimi.priority_1) ? kimi.priority_1 : []),
      ...(Array.isArray(kimi.priority_2) ? kimi.priority_2 : [])
    ].flatMap(item => Array.isArray(item.md_code_family) ? item.md_code_family : []).filter(Boolean);

    const priorities = [];
    for (const item of (Array.isArray(kimi.priority_1) ? kimi.priority_1 : [])) {
      priorities.push(`PRIORIDAD 1 · ${item.area}: ${item.reason || "alto impacto estético"}`);
    }
    for (const item of (Array.isArray(kimi.priority_2) ? kimi.priority_2 : [])) {
      priorities.push(`PRIORIDAD 2 · ${item.area}: ${item.reason || "refinamiento"}`);
    }
    for (const area of (Array.isArray(kimi.no_treatment) ? kimi.no_treatment : [])) {
      priorities.push(`SIN TRATAR · ${area}`);
    }

    state.plan = {
      filler, botox, threads, mdCodes: [...new Set(mdCodes)], priorities,
      provider: kimi.provider || "Kimi + MediaPipe",
      summary: kimi.full_face_summary || ""
    };
    return;
  }
  ...
  if (selected.has("midface") || state.fullFace) {
    filler.push({ area: "Soporte de tercio medio", ml: "evaluar según déficit visible" });
    mdCodes.push("Ck — familia mejilla / soporte");
    priorities.push("PRIORIDAD 1 · Foundation: soporte de tercio medio si la evaluación clínica confirma pérdida estructural.");
  }
  ...
  if (selected.has("chin") || state.fullFace) {
    if ((m.lowerThirdToFaceHeight || 0) < 0.34) {
      ...
      mdCodes.push("C — familia mentón");
      priorities.push("PRIORIDAD 2 · Contour: valorar mentón por proporción del tercio inferior.");
    }
  }

  if (selected.has("jaw") || selected.has("prejowl") || state.fullFace) {
    if ((m.jawToFaceWidth || 0) < 0.74 || (m.transverseAsymmetry || 0) > 0.08) {
      filler.push({ area: "Pre-jowl / continuidad mandibular", ml: "rango orientativo a definir clínicamente" });
      mdCodes.push("Jw — familia mandíbula / jowl");
      priorities.push("PRIORIDAD 2 · Contour: valorar continuidad mandibular sin sobredefinir.");
    }
  }

  if (selected.has("undereye") || state.fullFace) {
    filler.push({ area: "Ojeras / tear trough", ml: "solo si persiste tras soporte de midface" });
    mdCodes.push("Tt — familia tear trough");
    priorities.push("REFINEMENT · Ojeras solo después de valorar foundation del tercio medio.");
  }

  if (selected.has("lips")) {
    filler.push({ area: "Labios", ml: "conservador; definir tras evaluación clínica" });
    mdCodes.push("Lp — familia labios");
  }

  if (selected.has("perioral")) {
    filler.push({ area: "Nasolabial / perioral", ml: "solo si el soporte estructural no es suficiente" });
    mdCodes.push("NL / M — familias nasolabial y marionette");
  }

  if (selected.has("temples")) {
    filler.push({ area: "Sienes", ml: "según pérdida de volumen visible" });
    mdCodes.push("T — familia sien");
  }
  ...
  state.plan = { filler, botox, threads, mdCodes, priorities, provider: "MediaPipe (fallback)", summary: "" };
}
...
async function generateAiBestVersion() {
  ...
  try {
    ...
    if (live) {
      live.textContent = state.kimiPlan ? "Best Version lista · Kimi + IA activos · intensidad en vivo" : "Best Version lista · intensidad en vivo sin nuevas generaciones";
      live.classList.remove("prp-ai-error");
    }
