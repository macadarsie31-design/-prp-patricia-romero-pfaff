(() => {
"use strict";
console.info("PRP simulator build V23-download-100");
...
const PROFILE_CONFIG = {
  best: {
    ...
    planScale: 1.08,
    blendCap: 1.0,
    liveReady: "Simulación Mejor versión lista · intensidad 100%"
  }
};
...
function setupStrength() {
  ...
  if (label) label.textContent = "100% · Definido";
}
  ...
  overlay.innerHTML = `
    <div class="prp-lead-modal" role="dialog" aria-modal="true" aria-labelledby="prpLeadTitle">
      ...
      <form id="prpLeadForm">
        ...
        <a id="prpPdfFallback" href="#" target="_blank" rel="noopener" style="display:none;margin-top:10px;text-align:center;font-weight:750;color:#203a2f">Abrir / guardar informe PDF</a>
      </form>
    </div>`;
...
async function submitLeadAndDownload(e) {
  ...
  submit.textContent = "Preparando informe…";
  error.textContent = "";
  if (fallback) fallback.style.display = "none";

  let pdfInfo;
  try {
    // IMPORTANT: create and trigger the PDF before any network await.
    // This keeps the action inside the user's tap, which is required by iPhone/Safari.
    pdfInfo = createFullReportPdfBlob(patient);
    pdfInfo.url = triggerPdfDownload(pdfInfo.blob, pdfInfo.fileName, fallback);
  } catch (pdfErr) {
    console.error("PDF creation error", pdfErr);
    error.textContent = pdfErr?.message || "No pudimos crear el informe PDF.";
  ...
  try {
    submit.textContent = "Guardando tus datos…";
    const payload = {
      name: patient.name,
      ...
      simulation: "Full Face · Mejor versión · 100%",
    ...
    if (!response.ok) {
      ...
      throw new Error(data?.errors?.[0]?.message || "El PDF está listo, pero no pudimos guardar tus datos.");
    }

    submit.textContent = "Informe listo ✓";
    setTimeout(() => {
      closeLeadModal();
      form.reset();
      submit.disabled = false;
    ...
    error.textContent = (err?.message || "No pudimos guardar tus datos.") + " Podés abrir el PDF con el enlace de abajo.";
    submit.disabled = false;
    submit.textContent = "Reintentar guardar datos";
    if (fallback && pdfInfo?.url) {
      fallback.href = pdfInfo.url;
...
function createFullReportPdfBlob(patient) {
  ...
  paragraph("PRP · Dra. Patricia Romero Pfaff · SUR MULTI ESPACIO · Bernardo O'Higgins 5435, Córdoba, Argentina · WhatsApp +54 9 3512 64-1380", 8);
  const safeName = patient.name.normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-zA-Z0-9]+/g,"-").replace(/^-|-$/g,"").toLowerCase() || "paciente";
  const fileName = `PRP-Propuesta-Full-Face-${safeName}.pdf`;
  const blob = doc.output("blob");
  return { blob, fileName };
}

function triggerPdfDownload(blob, fileName, fallbackLink) {
  const url = URL.createObjectURL(blob);
  const isIOS = /iPad|iPhone|iPod/.test(navigator.userAgent) || (navigator.platform === "MacIntel" && navigator.maxTouchPoints > 1);

  // On iPhone/iPad the most reliable way to save a generated PDF is the native share sheet,
  // where the patient can choose “Save to Files”. It must be triggered directly from the tap.
  try {
    if (isIOS && typeof File === "function" && navigator.share && navigator.canShare) {
      ...
      if (navigator.canShare({ files: [file] })) {
        navigator.share({
          files: [file],
          title: "PRP · Informe Full Face",
          text: "Tu propuesta Full Face · Mejor versión"
        }).catch(err => {
    ...
    console.warn("Automatic PDF save failed", err);
  }

  // Always expose a fallback link. On iPhone it opens the PDF so it can be saved with Share → Save to Files.
  if (fallbackLink) {
    fallbackLink.href = url;
    fallbackLink.download = fileName;
    fallbackLink.style.display = "block";
    fallbackLink.textContent = isIOS ? "Abrir PDF si no apareció Guardar/Compartir" : "Abrir / guardar informe PDF";
  }

  // Safari may need the Blob URL for a while.
...
async function generateAiBestVersion() {
  ...
  try {
    ...
    setStatus("Simulación lista · intensidad 100%.");
