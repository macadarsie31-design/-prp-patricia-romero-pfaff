...
function buildPrompt(zones = [], analysis = null, clinicalPlan = null) {
  ...
  const planBlocks = clinicalPlan ? [
    `Priority 1: ${JSON.stringify(clinicalPlan.priority_1 || [])}`,
    `Priority 2: ${JSON.stringify(clinicalPlan.priority_2 || [])}`,
    `No treatment: ${JSON.stringify(clinicalPlan.no_treatment || [])}`,
    `Procedures: ${JSON.stringify(clinicalPlan.procedures || {})}`,
    `Full-face summary: ${clinicalPlan.full_face_summary || ""}`,
    `Safeguards: ${JSON.stringify(clinicalPlan.safeguards || [])}`
  ].join("\n- ") : "No Kimi structured plan available.";
...
The frontend performed a 478-point MediaPipe Face Landmarker scan.
...
KIMI STRUCTURED CLINICAL PLAN
A multimodal analysis engine already evaluated the selfie.
Treat this structured plan as a strong planning guide, while still obeying all identity-lock and naturalness rules:
- ${planBlocks}

If the structured plan recommends no treatment in an area, do not visibly change that area.
If the plan says a refinement should happen only after foundation support, reflect that logic in the final appearance.
...
export async function onRequestPost(context) {
  try {
    ...
    const clinicalPlan = body?.clinicalPlan && typeof body.clinicalPlan === "object" ? body.clinicalPlan : null;
    form.append("prompt", buildPrompt(zones, analysis, clinicalPlan));
