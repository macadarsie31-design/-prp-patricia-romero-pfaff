...
const MODEL_ID = "Qwen/Qwen-Image-Edit-2511";
const DEFAULT_FAL_MODEL = "fal-ai/qwen-image-edit-2511";
...
async function getFalModelId(token) {
  try {
    const r = await fetch(
      `https://huggingface.co/api/models/${encodeURIComponent(MODEL_ID)}?expand=inferenceProviderMapping`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    if (!r.ok) return DEFAULT_FAL_MODEL;
    const info = await r.json();
    const mapping = info?.inferenceProviderMapping || info?.inference_provider_mapping;
    const fal = Array.isArray(mapping)
      ? mapping.find(x => (x.provider || x.providerId) === "fal-ai")
      : mapping?.["fal-ai"];
    return fal?.providerId || fal?.provider_id || fal?.modelId || fal?.model_id || DEFAULT_FAL_MODEL;
  } catch {
...
export async function onRequestPost(context) {
  try {
    ...
    const falModel = await getFalModelId(token);
