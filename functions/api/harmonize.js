...
//
// v5: v4's "very light natural makeup" instruction backfired badly on
// klein-4b — instead of subtle, it produced blotchy orange blush patches and
// heavy dark lipstick. A 4-step distilled model can't reliably hit "barely
// noticeable" — it either skips an instruction or overshoots it. Makeup
// instruction removed; back to pure harmonization only. This is now the 4th
// prompt-only iteration that hits a real ceiling of this specific model
// (see chat: nasolabial fold, tear trough, and now makeup subtlety all
// failed or overshot across rounds) — recommend testing flux-2-max next
// rather than continuing to tune klein-4b's prompt further.
function buildPrompt() {
  return [
    "Apply these specific visible corrections to this face: 1) Fill and soften the nasolabial folds (the creases running from the nose to the corners of the mouth) so they are much less visible. 2) Fully correct the dark circles and hollow tear-trough shadow under the eyes — brighten and even out that area so it blends smoothly into the cheek, no dark shadow left. 3) Add clearly visible volume and hydration to the lips, fuller than the original but keeping the same lip shape, Cupid's bow and natural lip color. 4) Lift and add volume to the cheeks/cheekbones (malar area) for a more contoured, lifted midface.",
    ...
    "All of this must be a photorealistic edit of the EXACT SAME PERSON, sharp and highly detailed, not blurry, not textured with fake wrinkles or grain/noise anywhere on the skin. Keep the same face shape, bone structure, eye color, ethnicity, hairline, hair color, hairstyle, ears, skin tone, head position, facial expression, camera angle, perspective, background, lighting and clothing exactly as in the original photo.",
    ...
    "Do not: swap identity, change bone structure, change eye color, create an unnatural or surgical-looking canthal change, create duck lips or overfilled cheeks, freeze the expression, create a tiny generic 'Barbie' nose, add plastic or waxy skin, add fake grain/texture/wrinkles anywhere on the skin, apply any makeup (no blush, no bronzer, no lipstick, no eyeshadow, no foundation), change hair color or hairstyle, change the background, change the lighting, or change the camera angle."
  ].join(" ");
}
