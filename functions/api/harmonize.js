...
// natural-language instruction with the identity-preservation rule stated
// first AND last (primacy + recency matter a lot for a 4-step model).
function buildPrompt() {
  return [
    "Photorealistic edit of the EXACT SAME PERSON in the uploaded photo, sharp and highly detailed, not blurry or soft. Do not change identity: keep the same face shape, bone structure, eye color, ethnicity, hairline, hair color, hairstyle, ears, skin tone, head position, facial expression, camera angle, perspective, background, lighting and clothing exactly as in the original photo.",
    "Simulate a natural, high-end, non-surgical full-face aesthetic harmonization performed by an expert injector using dermal filler, neuromodulator and thread-lift techniques (MD Codes approach), applied clearly and visibly where it benefits this specific face: relaxed and smoother forehead and glabella lines; a fresh, rested, subtly lifted eye area with reduced tear-trough shadows and a visible, elegant lateral brow/canthal lift (foxy-eye effect) while keeping the same eye color and the person's natural eye spacing; well-defined, lifted cheekbones with clear malar contouring and midface support; a very subtle lift and refinement of the nasal tip and bridge continuity, keeping the person's recognizable nose identity and ethnicity; softened nasolabial and marionette folds; naturally hydrated and refined lips with the same lip shape and Cupid's bow; a clearly more defined, contoured jawline with reduced jowls and a visible, elegant V-shaped lower-face contour; a harmonized, well-projected chin; and healthier, more even skin with visible real pores and texture — no makeup added, no beauty filter.",
    "The facial marking / contouring should be clearly visible in a side-by-side before-and-after comparison — a confident, well-defined harmonization result, not a barely-noticeable change — while still looking anatomically plausible, sharp, photorealistic and unmistakably like the same person.",
    "Do not: swap identity, change bone structure, change eye color, create an unnatural or surgical-looking canthal change, create duck lips or overfilled cheeks, freeze the expression, create a tiny generic 'Barbie' nose, add plastic or waxy skin, blur or soften the image, change hair color or hairstyle, change the background, change the lighting, or change the camera angle."
  ].join(" ");
}
...
export async function onRequestPost(context) {
  try {
    ...
    const width = clampInt(body?.width, 256, 1024, 900);
    const height = clampInt(body?.height, 256, 1024, 900);
